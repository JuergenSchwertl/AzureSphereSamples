...
// connect an Azure Sphere device to Azure IoT Hub. To use this sample, you must first
// copy the DPS scop ID to required "CmdArgs" with the DPS scope id and add our Azure IoT Hub DNS address to 
// the "AllowedConnections" properties for the Manifest (app_manifest.json) .
...
static void SetLedRate(const struct timespec * pBlinkRate)
{
    ...
    // report changed blink rate as property to Azure IoT Hub
...
static void SendTelemetryMessage(void)
{
    if (connectedToIoTHub) {
        ...
        AzureIoT_SendTextMessage("Hello from Azure Sphere");
...
static void DeviceTwinUpdate(JSON_Object *desiredProperties)
{
	if (json_object_has_value_of_type(desiredProperties, cstrLedBlinkRateProperty, JSONNumber))
	{
		...
		Log_Debug("[DeviceTwinUpdate] Received desired value %zu for %s, setting it to %zu.\n",
			desiredBlinkRate, cstrLedBlinkRateProperty, blinkIntervalIndex);
